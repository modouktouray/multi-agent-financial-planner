...
- **`sagemaker/`** - SageMaker serverless endpoint for embeddings 
- **`ingestion/`** - S3 Vectors, Lambda, and API Gateway for document ingestion 
- **`researcher/`** - App Runner service for AI researcher agent 
- **`database/`** - Aurora Serverless v2 PostgreSQL with Data API 
- **`agents/`** - Lambda functions for agent orchestra 
- **`frontend/`** - API Lambda and frontend infrastructure 
- **`observability/`** - LangFuse and monitoring setup 
...
cd terraform/sagemaker  # (or 3_ingestion, 4_researcher, etc.)
